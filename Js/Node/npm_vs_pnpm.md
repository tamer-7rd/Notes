...
1. **Глобальное хранилище**: Все пакеты из **всех твоих проектов** хранятся в одном месте (`~/.pnpm-store`)
   - Когда ты устанавливаешь зависимости в любом проекте, они попадают в это хранилище
   - Если несколько проектов используют одну и ту же версию пакета (например, `react@18.2.0`), она хранится **только один раз**
   - Каждая версия пакета хранится отдельно (например, `react@18.2.0` и `react@17.0.0` — это разные записи)
2. **Жёсткие ссылки (hard links)**: В `node_modules` каждого проекта создаются ссылки на файлы из хранилища (не копии!)
