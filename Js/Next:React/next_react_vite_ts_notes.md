...
- В Next бандлер встроен. По умолчанию используется Webpack, также доступен Turbopack (написан на Rust, быстрее, но пока опционален через флаг `--turbo`).
...
3. Next даёт встроенный Webpack (и опционально Turbopack) → тебе не нужно писать код под сборщик.
