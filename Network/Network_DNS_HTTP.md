...
Это глобальная распределенная система и сетевой протокол.

Что это: Это вся совокупность правил, стандартов, серверов по всему миру и механизмов их взаимодействия.

Задача: Обеспечить работу всей инфраструктуры превращения имен (https://www.google.com/search?q=google.com) в IP-адреса.

Включает в себя: Протоколы передачи данных (UDP/TCP порт 53), корневые серверы, кэширующие серверы провайдеров и все типы записей.

**Name Servers (NS)**

Это конкретные серверы (софт и железо), которые являются частью системы DNS. DNS servers == Name servers

Что это: Это узлы (серверы), на которых физически хранятся файлы с настройками твоего домена (зоновые файлы).

Задача: Отвечать на запросы других компьютеров. Когда кто-то спрашивает: «Где найти сайт mysite.com?», именно Name Server открывает свою базу и выдает ответ.

Роль: В системе DNS существует иерархия, и Name Servers в ней бывают разных уровней (корневые, TLD, авторитативные). Когда ты прописываешь NS у регистратора, ты указываешь Authoritative Name Servers — то есть «серверы, имеющие полномочия» отвечать за твой домен.
